...
public class CharacterStatus : MonoBehaviour
{
    ...
    public Timer invincibillytyTimer = new Timer(1f);   //무적 타이머 1초
    ...
    void Update()
    {
        ...
        invincibillytyTimer.Update(deltaTime);
        ...
        if (!invincibillytyTimer.IsRunning())       //무적 타이머 동작 체크
        {
            invincible = false;
        }
    ...
    public void ApplyDamage(int damage, Vector3 positon)    //데미지를 받았을때 잠시 무적 시간을 주고 스턴 처리
    {
        if (!invincible)
        {
