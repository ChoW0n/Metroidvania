...
public class CharacterMovement : MonoBehaviour
{
    ...
    const float k_GroundedRaius = 0.2f;                 //땅에 닿았는지 확인하는 겹침 원의 반경
    ...
    private void FixedUpdate()              //FixedUpdate에서 이동 구현을 한다.
    {
        ...
        Collider2D[] groundColiiders = Physics2D.OverlapCircleAll(m_GroundCheck.position, k_GroundedRaius, m_WhatIsGround);
        ...
        bool wasWallSliding = m_IsWallSlidding;
        m_IsWallSlidding = false;
        m_IsWall =false;


        //WallCheck 위치에서 벽으로 지정된 것과 겹치는지 확인
        Collider2D[] WallColiiders = Physics2D.OverlapCircleAll(m_WallCheck.position, k_GroundedRaius, m_WhatIsGround);
        for (int i = 0; i < WallColiiders.Length; i++)
        {
            if (WallColiiders[i].gameObject != gameObject)
            {
                ...
                if (!m_IsGrounded && !status.isDashing && m_Rigidbody2D.velocity.y < 0)
                {
                    ...
                    if (!wasWallSliding)         //이전 프레임이 벽이 아니였다면 체크하여 돌려서 벽을 슬라이딩 하고 검출 하게 한다.
                    {
            ...
            if (limitVelOnWallJump)
            {
                ...
                {
                    status.canMove = true;
                    m_Rigidbody2D.velocity = new Vector3(10.0f * transform.localScale.x, m_Rigidbody2D.velocity.y);
                }
                else if (jumpWallDistX < -2)
                {
                    status.canMove = false;
                    m_Rigidbody2D.velocity = new Vector3(10.0f, m_Rigidbody2D.velocity.y);
                }
        ...
        if (!m_IsWallSlidding && wasWallSliding)
        {
    ...
    public void Move(float move, bool jump, bool dash)
    {
        if (status.canMove)      //클래스에서 이동이 가능하다고 하면
        {
            ...
            else if (m_GroundCheck || m_AirControl)  //플레이어가 땅에 닿았거나 공중 제어가 켜져있을때
            {
            ...
            {
                if (!oldWallSlidding && m_Rigidbody2D.velocity.y < 0 || status.isDashing)    //벽에 붙어서 슬아이딩 되는 순간
                {
                    m_IsWallSlidding = true;
                    m_WallCheck.localPosition = new Vector3(-m_WallCheck.localPosition.x, m_WallCheck.localPosition.y, 0);
                    Flip();
    ...
    private void Flip()                     //플립 함수를 호출하여 세팅
    {
